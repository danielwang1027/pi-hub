...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  button, select, input, textarea { font-family: 'Montserrat', sans-serif; }
...
export default function App() {
  ...
  useEffect(() => {
    try { if (sessionStorage.getItem("pi_authed") === "1") setAuthed(true); } catch(e) {}
  }, []);
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F7F9FC", fontFamily:"'Montserrat', sans-serif" }}>
      <style>{css}</style>

      {/* Password Gate */}
      {!authed && (
        <div style={{ position:"fixed", inset:0, background:"linear-gradient(135deg, #1a56f0 0%, "+MOLOCO_BLUE+" 40%, "+MOLOCO_NAVY+" 100%)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:24 }}>
          <div style={{ background:"#fff", borderRadius:20, padding:"40px 36px", width:"100%", maxWidth:380, boxShadow:"0 20px 60px rgba(0,0,0,0.2)", textAlign:"center" }}>
            ...
            <input
              ...
              style={{ width:"100%", padding:"11px 14px", borderRadius:10, border:`1.5px solid ${pwError ? "#E53E3E" : "#DDE4F0"}`, fontSize:14, outline:"none", marginBottom:8, textAlign:"center" }}
            />
            {pwError && <div style={{ fontSize:12, color:"#E53E3E", marginBottom:8 }}>Incorrect password</div>}
            <button onClick={handleLogin}
              style={{ width:"100%", padding:"12px", borderRadius:10, background:MOLOCO_BLUE, border:"none", cursor:"pointer", color:"#fff", fontSize:14, fontWeight:700 }}>
              Enter
            </button>
          </div>
        </div>
            ...
            <div style={{ maxWidth:1100, margin:"0 auto", padding:"12px 32px" }} className="sticky-bar">
              ...
              <div style={{ position:"relative", marginBottom:10 }}>
                ...
                <input type="text" placeholder="Search by title, summary or platform..."
                  ...
                  style={{ width:"100%", padding:"9px 14px 9px 38px", borderRadius:10, border:"1.5px solid #DDE4F0", background:"#fff", fontSize:13, color:"#0F1729", outline:"none" }} />
                {searchQuery && (
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <select value={selCat} onChange={e => setSelCat(e.target.value)}
                  style={{ fontSize:13, padding:"6px 12px", borderRadius:8, border:"1.5px solid #DDE4F0", background:"#fff", color:"#333", outline:"none", cursor:"pointer" }}>
          ...
          {submitOpen && (
            <div onClick={() => setSubmitOpen(false)}
              ...
              <div onClick={e => e.stopPropagation()}
                style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:520, padding:28, boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
                ...
                {submitDone ? (
                  <div style={{ textAlign:"center", padding:"32px 0" }}>
                    ...
                    <button onClick={() => setSubmitOpen(false)}
                      style={{ marginTop:20, padding:"10px 24px", borderRadius:999, background:MOLOCO_BLUE, border:"none", cursor:"pointer", color:"#fff", fontSize:13, fontWeight:600 }}>
                      Close
                    </button>
                  </div>
                ) : (
                  <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                        ...
                        {["X","Kakao","Naver","Samsung","Xiaomi","Pinterest","Yahoo","LINE","Other"].map(pi => {
                          ...
                          return (
                            <button key={pi} onClick={() => setSubmitForm(f => ({ ...f, platforms: active ? f.platforms.filter(p => p !== pi) : [...f.platforms, pi] }))}
                              style={{ padding:"4px 12px", borderRadius:999, fontSize:12, fontWeight:600, cursor:"pointer", border:`1.5px solid ${active ? color : "#DDE4F0"}`, background: active ? color : "#fff", color: active ? (pi==="Kakao"?"#333":"#fff") : "#555" }}>
                              {pi}
                            </button>
                          );
                        })}
                    ...
                    <div>
                      ...
                      <select value={submitForm.category} onChange={e => setSubmitForm(f => ({ ...f, category: e.target.value }))}
                        style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:"1.5px solid #DDE4F0", fontSize:13, color:"#0F1729", outline:"none" }}>
                    ...
                    <div>
                      ...
                      <textarea placeholder="Describe the update in detail..." value={submitForm.summary} onChange={e => setSubmitForm(f => ({ ...f, summary: e.target.value }))}
                        rows={4} style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:"1.5px solid #DDE4F0", fontSize:13, color:"#0F1729", outline:"none", resize:"vertical" }} />
                    </div>
